// all ecs code belongs to @mreinstein - https://github.com/mreinstein/ecs
...
function addComponentToEntity(
  ...
  if (!entity[componentName]) world.stats.componentCount[componentName] += 1;
...
function getEntities(world, componentNames, listenerType) {
  ...
  if (!world.filters[filterId])
    world.filters[filterId] = world.entities.filter((e) =>
      _matchesFilter(filterId, e)
    );
  ...
  if (world.stats.systems[systemIdx]) {
    ...
    world.stats.systems[systemIdx].filters[filterId] +=
      world.filters[filterId].length;
  }
...
function _matchesFilter(filterId, entity, componentIgnoreList = []) {
  ...
  for (const componentId of componentIds) {
    ...
    if (!componentId.startsWith("!") && !entity[componentId]) return false;
  }
...
function addSystem(world, fn) {
  ...
  if (!system.onFixedUpdate) system.onFixedUpdate = function () {};

  if (!system.onPreUpdate) system.onPreUpdate = function () {};

  if (!system.onUpdate) system.onUpdate = function () {};

  if (!system.onPostUpdate) system.onPostUpdate = function () {};
...
function _resetStats(world) {
  ...
  for (const system of world.stats.systems) {
    ...
    for (const filterId in system.filters) system.filters[filterId] = 0;
  }
...
function _removeComponent(world, entity, componentName) {
  ...
  for (const filterId in world.filters) {
    ...
    if (_matchesFilter(filterId, entity) && !filter.includes(entity)) {
      ...
      if (filterIdx >= 0) removeItems(filter, filterIdx, 1);
    }
  }
}
...
function cleanup(world) {
  ...
  for (let i = 0; i < world.removals.components.length; i++) {
    const [entityIdx, componentName] =
      world.removals.components[i].split("__@@ECS@@__");
  ...
  for (const entityIdx of world.removals.entities) {
    ...
    for (const filterId in world.filters) {
      const filter = world.filters[filterId];
      const idx = filter.indexOf(entity);
      if (idx >= 0) removeItems(filter, idx, 1);
    }
  }
